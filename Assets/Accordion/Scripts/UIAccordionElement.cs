...
namespace UnityEngine.UI
{
	...
	public class UIAccordionElement : Toggle {
		...
		protected override void OnValidate()
		{
			...
			#endif
		// For Enabling CP Points with in this SP;
		public void EnableCP_Points(bool state)
		{
			gendertog.genderToggleObject.SP_Controller(state);
			...
			for (int i = 1; i < gameObject.transform.childCount; i++) {
				...
				GameObject CP =	GameObject.Find(gameObject.transform.GetChild (i).transform.GetChild (1).GetComponent<Text> ().text+"_CP");
			
				try
				{	if (CP.tag == "MaleCorrespondingPoints" || CP.tag == "FemaleCorrespondingPairs") {
					CP.GetComponent<MeshRenderer> ().enabled = state;
				}
		...
		{
			if(!UniversalClass.IsDictionaryEnabled){
				EnableCP_Points (state);
			}
